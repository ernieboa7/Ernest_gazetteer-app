...
(() => {
  ...
  // 📍 Handle Coordinate Search (now includes country info)
  async function handleCoordSearch(lat, lng) {
    try {
      ...
      // get country code via OpenCage for coordinate searches
      let countryCode = null, country = null;
      if (nearby?.countryName) {
        const geo = await getJSON('./php/opencage.php', { q: nearby.countryName, limit: 1 });
        const best = geo?.results?.[0];
        countryCode =
          best?.components?.country_code?.toUpperCase?.() ||
          best?.components?.country?.toUpperCase?.();
        if (countryCode) country = await fetchCountryInfo(countryCode);
      }

      updatePlaceInfo({ lat, lng, nearby, countryCode });
      updateTimeInfo(tz);
      updateWeatherInfo(weather);
      updateCountryInfo(country);
...
/* Gazetteer SPA – Final Stable Version
 * Works with: php/opencage.php, php/geonames.php, php/weather.php, php/restcountries.php
 * Features: Full info for place search, coordinates, and "Use My Location"
 */

/* Gazetteer App - Final Optimized Version
 * Uses Bootstrap 5 + Leaflet
 * Proxies: php/opencage.php, php/geonames.php, php/weather.php, php/restcountries.php
 */
(() => {
  ...
  function showLoader(show = true) {
    el.loader.classList.toggle("d-none", !show);
  }
  function notify(msg) {
  ...
  function renderHistory() {
    ...
    for (const it of list) {
      ...
      li.innerHTML = `<span>${it.name ?? it.lat + ", " + it.lng}</span>
  ...
  async function getJSON(url, params = {}) {
    ...
    Object.entries(params).forEach(
      ([k, v]) => v !== undefined && v !== null && u.searchParams.append(k, v)
    );
  ...
  function updatePlaceInfo(place) {
    ...
    const nearbyCity = place.nearby?.name
      ? `${place.nearby.name}, ${place.nearby.countryName}`
      : "—";
    const lines = [
      `<div><strong>Selected:</strong> ${place.displayName ?? "—"}</div>`,
      `<div><strong>Coordinates:</strong> ${
        place.lat?.toFixed?.(5) ?? place.lat
      }, ${place.lng?.toFixed?.(5) ?? place.lng}</div>`,
      `<div><strong>Nearest place:</strong> ${nearbyCity} (${
        place.nearby?.distance?.toFixed?.(1) ?? "—"
      } km)</div>`,
      place.countryCode
        ? `<div><strong>Country code:</strong> ${place.countryCode}</div>`
        : "",
    ].filter(Boolean);
    el.placeInfo.innerHTML = lines.join("");
  }
  ...
  function updateWeatherInfo(weather) {
    ...
    const lines = [
      ...
      `<div><strong>Conditions:</strong> ${weather.description}</div>`,
      weather.wind
        ? `<div><strong>Wind:</strong> ${weather.wind} m/s</div>`
        : "",
    ].filter(Boolean);
    el.weatherInfo.innerHTML = lines.join("");
  }
  ...
  const fetchTimezone = async (lat, lng) =>
    getJSON("./php/geonames.php", { endpoint: "timezoneJSON", lat, lng }).then(
      (tz) =>
        !tz || tz.status
          ? null
          : { timezoneId: tz.timezoneId, gmtOffset: tz.gmtOffset, localTime: tz.time }
    );

  const fetchWeather = async (lat, lng) =>
    getJSON("./php/weather.php", { lat, lon: lng }).then((w) => {
      if (!w || w.cod !== 200) return null;
      const cur = w.main || {};
  ...
  async function handlePlaceSearch(query) {
    try {
      ...
      setMapView(lat, lng, displayName);
      const [nearby, tz, weather, country] = await Promise.all([
        fetchNearby(lat, lng),
        fetchTimezone(lat, lng),
        fetchWeather(lat, lng),
        countryCode ? fetchCountryInfo(countryCode) : null,
      ]);
  ...
  async function handleCoordSearch(lat, lng) {
    try {
      ...
      const countryName = nearby?.countryName || null;
      const reverse = !countryName
        ? await getJSON("./php/opencage.php", { lat, lng, limit: 1 })
        : null;
      const best = reverse?.results?.[0];
      const countryCode =
        best?.components?.country_code?.toUpperCase?.() ||
        best?.components?.country?.toUpperCase?.();
      const country = countryCode ? await fetchCountryInfo(countryCode) : null;
  ...
  el.btnSearchCoords.addEventListener("click", () =>
    handleCoordSearch(el.latInput.value, el.lngInput.value)
  );
  el.btnUseMyLocation.addEventListener("click", () => {
    if (!navigator.geolocation) return notify("Geolocation not supported.");
    navigator.geolocation.getCurrentPosition(
  ...
  // 🏠 Home button (reset + collapse sidebar)
  document.getElementById("btnHome")?.addEventListener("click", (e) => {
    ...
    document.getElementById("countryInfo").innerHTML = "<em>No country info.</em>";
    el.placeInput.value = el.latInput.value = el.lngInput.value = "";
    notify("Map reset to default view.");
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          const [nearby, tz, weather] = await Promise.all([
            fetchNearby(lat, lng),
            fetchTimezone(lat, lng),
            fetchWeather(lat, lng),
          ]);
