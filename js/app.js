...
(() => {
  ...
  // 📍 Handle Coordinate Search (Always includes country info)
  async function handleCoordSearch(lat, lng) {
    try {
      if (typeof lat === 'string') lat = parseFloat(lat);
      if (typeof lng === 'string') lng = parseFloat(lng);
      ...
      const nearby = await fetchNearby(lat, lng);
      let countryName = nearby?.countryName || null;

      // Fallback: reverse geocode if GeoNames doesn’t return a country
      if (!countryName) {
        const reverse = await getJSON('./php/opencage.php', { lat, lng, limit: 1 });
        const best = reverse?.results?.[0];
        countryName = best?.components?.country || null;
      }
      ...
      let countryCode = null, country = null;
      if (countryName) {
        const geo = await getJSON('./php/opencage.php', { q: countryName, limit: 1 });
        const best = geo?.results?.[0];
        countryCode =
          best?.components?.country_code?.toUpperCase?.() ||
          best?.components?.country?.toUpperCase?.();
