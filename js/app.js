...
/* Gazetteer App – Stable Mobile Version
 * Uses Bootstrap 5 + Leaflet
 * Proxies:
 *   php/opencage.php
 *   php/geonames.php
...
(() => {
  ...
  // ✅ Reliable OpenStreetMap tiles (best for mobile)
  ...
  function setMapView(lat, lng, label, zoom = 8) {
  ...
  async function fetchNearby(lat, lng) {
    const data = await getJSON("./php/geonames.php", {
      endpoint: "findNearbyPlaceNameJSON",
      lat,
      lng,
      radius: 20,
  ...
  async function fetchWeather(lat, lng) {
    const w = await getJSON("./php/weather.php", {
      lat,
      lon: lng,
    });
  ...
  function updatePlaceInfo(place) {
    ...
    el.placeInfo.innerHTML = `
      <div><strong>Selected:</strong> ${place.displayName ?? "—"}</div>
      <div><strong>Coordinates:</strong> ${place.lat}, ${place.lng}</div>
      <div><strong>Nearest:</strong> ${
        place.nearby?.name ?? "—"
      }</div>
    `;
  }
  ...
  function updateTimeInfo(info) {
    ...
    el.timeInfo.innerHTML = `
      <div><strong>Local time:</strong> ${dt.toLocaleString()}</div>
      <div><strong>Timezone:</strong> ${info.timezoneId}</div>
    `;
  }
  ...
  function updateCountryInfo(country) {
    ...
    elCountry.innerHTML = `
      <div class="text-center mb-2">
        <img src="${country.flag}" width="80" class="border rounded">
      </div>
      <div><strong>Country:</strong> ${country.name}</div>
      <div><strong>Capital:</strong> ${country.capital}</div>
      <div><strong>Population:</strong> ${country.population}</div>
  ...
  async function handlePlaceSearch(query) {
    try {
      ...
      const [nearby, tz, weather, country] =
        await Promise.all([
          fetchNearby(lat, lng),
  ...
  el.form.addEventListener("submit", (e) => {
    e.preventDefault();
    handlePlaceSearch(
      el.placeInput.value.trim()
    );
  });

  el.btnSearchCoords.addEventListener(
    "click",
    () =>
      handleCoordSearch(
        el.latInput.value,
  ...
  el.btnUseMyLocation.addEventListener(
    "click",
    () => {
      if (!navigator.geolocation)
        return notify(
